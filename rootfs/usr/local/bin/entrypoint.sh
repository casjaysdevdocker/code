...
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for code
...
#SERVICE_USER="${SERVICE_USER:-code}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-code}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    if [ "$ENTRYPOINT_MESSAGE" = "yes" ]; then
      echo "Executing entrypoint script for code"
    fi
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
